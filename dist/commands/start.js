...
import { getWorktreeEntry, getAllocatedPortBases, getAllocatedProjectIds, registerWorktree, } from "../lib/registry.js";
import { extractPorts, getPortBase, allocatePortBase, buildPortMap } from "../lib/ports.js";
import { generateConfig, deriveProjectId, } from "../lib/template.js";
...
export async function startCommand(ctx) {
    ...
    if (!config) {
        throw new CommandError("No \"supabase-worktree\" config found in package.json", `Run ${chalk.cyan("sb-worktree init")} to set up your project first.`);
    }
    ...
    if (!existsSync(templatePath)) {
        ...
        if (hasConfigToml) {
            throw new CommandError(`Template not found: ${config.configTemplate}`, `A config.toml exists but hasn't been templated yet.\n  Run ${chalk.cyan("sb-worktree init")} to create the template.`);
        }
        else {
            throw new CommandError(`Template not found: ${config.configTemplate}`, `No Supabase config found in this project.\n  Run ${chalk.cyan("supabase init")} first, then ${chalk.cyan("sb-worktree init")}.`);
        }
    }
    // 3. Check if already running
    const existing = await getWorktreeEntry(projectRoot);
    if (existing) {
        console.log(chalk.yellow(`⚠ This worktree already has an active instance (port base: ${existing.portBase})`));
        if (interactive) {
            ...
            console.log(chalk.yellow("  Use --force to replace, or run 'sb-worktree stop' first."));
            process.exitCode = 1;
            return;
        }
    }
    ...
    if (extractedPorts.length === 0) {
        throw new CommandError(`No port definitions found in ${config.configTemplate}`, "The template file exists but contains no port = <number> lines.\n  Check that the template is a valid Supabase config.toml.");
    }
    if (verbose) {
        console.log(chalk.dim(`  Found ${extractedPorts.length} port(s) in template: ` +
    ...
    const newBase = allocatePortBase(allocatedBases, templatePortBase, config.portBlockSize);
    const portMap = buildPortMap(extractedPorts, newBase);
    console.log(chalk.dim("  Port allocation:"));
    console.log(chalk.dim(formatPortTable(portMap, extractedPorts)
        .split("\n")
        .map((l) => "    " + l)
        .join("\n")));
    // 6. Derive project_id
    ...
    try {
        ...
        throw new CommandError("Could not determine git branch or repository name", `Make sure you're inside a git repository.\n  Run ${chalk.cyan("git init")} if this is a new project.`);
    }
    ...
    const configOutputPath = join(projectRoot, config.configTemplate.replace(".template", ""));
    ...
    if (modifiedEnvs.length > 0) {
        console.log(chalk.green(`  ✓ Updated ${modifiedEnvs.length} .env file(s):`));
    ...
    try {
        ...
        throw new CommandError(`Supabase failed to start: ${msg}`, "The config and .env files have been updated.\n" +
