...
export async function instructionsCommand(ctx: CommandContext): Promise<void> {
  ...
  console.log(
    chalk.dim("  supabase-worktree lets you run multiple Supabase instances\n") +
  ...
  if (!hasPkgJson) {
    ...
    printStep(5, "Commit the template and config changes",
...
function printCheck(ok: boolean, label: string): string {
  return ok
    ? chalk.green(`    ✓ ${label}`)
    : chalk.yellow(`    ✗ ${label}`) + chalk.dim(" (missing)");
}

function printStep(n: number, description: string, command: string): void {
